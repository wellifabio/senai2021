...
public class Arquivo {
	...
	public boolean criar(ArrayList<String> data, boolean append) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (temp[0].toLowerCase().equals(antigo.toLowerCase())) {
					...
					// Outro modo de fazer
					/*
					 * temp[0] = atual; for(int i = 0; i < temp.length; i++) { bwtemp.write(temp[i]
					 * + " "); } bwtemp.write("\r\n");
					 */
					conta++;
				} else {
	...
	//Metodo de exclusão de registro utilizando ArrayList
	public void excluirList(String busca) {

		//Aloca o conteudo no arquivo em um arraylist
		ArrayList<String> conteudo = ler();
		
		//Remove o nome desejado da lista
		conteudo.remove(busca);
		
		//Sobrescreve o arquivo utilizando o arraylist atualizado
		criar(conteudo, false);
		
	}
	
	// Metodo de exclusão de registro
	// Recebe o nome a ser excluido
	public boolean excluir(String busca) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				// Verifica o primeiro nome comparando com o parametro antigo
				if (!temp[0].toLowerCase().equals(busca.toLowerCase())) {
					bwtemp.write(line + "\r\n");
				}else {
					conta++;
				}
